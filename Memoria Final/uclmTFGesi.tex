...
\usepackage{tikz,marginnote,pgfplots}
\usepackage{tikz-cd}
\usetikzlibrary{arrows,arrows.meta}
\usetikzlibrary{babel,shapes.misc,positioning}
...
% -------------------------
\tituloPrimera{Desarrollo y Evaluación de un Modelo de Consumo \newline Energético para Procesadores ARM}
%\tituloPrimera{Caracterización del Consumo Energético en \newline Plataformas IoT para Aplicaciones Intensivas en Cómputo} % 1ª Línea
%\tituloSegunda{EN PLATAFORMAS IOT PARA APLICACIONES INTENSIVAS EN CÓMPUTO} % OPT.: Para títulos largos.
\tituloCorto{Modelo de Consumo Energético para Procesadores ARM} % Título corto (mostrado en pág. de créditos)
\autor{Antonio Fernando Mateo Francés}
\email{antoniofernando.mateo@alu.uclm.es}					
...
% (EN:MASTER IN COMPUTING ENGINEERING)
\tipoDoc{MÁSTER EN INGENIERÍA INFORMÁTICA} % (EN: BACHELOR DISSERTATION)
% Si las fechas se desean en inglés hay que ponerla explícita.
\mesTF{Octubre}        	% Mes de defensa (incluir siempre)
\monthTF{October}        	% En inglés (incluir siempre)
...
\input{acronimos} % [OPT:] Notación empleada.
...
\include{./anexos/AnexoCodigo} % Apéndice A (opcionales)
%\include{./anexos/AnexoTablas} % Apéndice A (opcionales)
%\include{./anexos/AnexoValidacion} % Apéndice A (opcionales)
\include{./anexos/AnexoMiscelaneos} % Apéndice A (opcionales)
